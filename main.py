...
if os.environ.get("DISCORD_TOKEN") is None:
    raise RuntimeError(
        "Fatal error: the DISCORD_TOKEN environment variable has not been set!"
    )

if os.environ.get("OWNER_ID") is None:
    raise RuntimeError(
        "Fatal error: the OWNER_ID environment variable has not been set!")

bot = ZEDROGAMES(command_prefix="!",
                 help_command=None,
